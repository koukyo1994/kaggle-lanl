...
class BottleNeck(chainer.Chain):
    ...
    def __init__(self, n_in, n_mid, n_out, stride=1, use_conv=False):
        ...
        with self.init_scope():
            self.conv1 = L.Convolution1D(n_in, n_mid, 1, stride, 0, True, w)
            self.bn1 = L.BatchNormalization(n_mid)
            self.conv2 = L.Convolution1D(n_mid, n_mid, 3, 1, 1, True, w)
            self.bn2 = L.BatchNormalization(n_mid)
            self.conv3 = L.Convolution1D(n_mid, n_out, 1, 1, 0, True, w)
...
class ResNet(chainer.Chain):
    ...
    def __init__(self, n_class=1, n_blocks=[3, 4, 6, 3]):
    ...
    def __call__(self, x):
        ...
        h = self.res5(h)
        h = self.res6(h)
        h = F.average_pooling_1d(h, h.shape[2:])
        h = self.fc7(h)
        return h
