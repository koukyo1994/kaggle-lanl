...
def main(filename="train_wave_0.csv"):
    train = pd.read_csv(DataPath / 'train_wave_split' / filename)
    max_row = len(train)
    segments = int(np.ceil(max_row / rows))
    eq_num = 0
    tail = np.inf
    for segment_num in tqdm(range(segments)):
        ...
        if tail < head:
            ...
            if segment_num < segments - 1:
                ...
                if min_ind != rows * (segment_num + 1) - 1:
                    thisSegment['eq_num'] = [eq_num] * (
                        min_ind - rows * segment_num + 1) + [eq_num + 1] * (
                            rows - (min_ind - rows * segment_num + 1))
                    eq_num += 1
                else:
                    thisSegment['eq_num'] = eq_num
                tail = thisSegment['time_to_failure'][rows * (segment_num + 1)
                                                      - 1]
            else:
                thisSegment['eq_num'] = eq_num
        ...
        thisSegment.to_csv(
            DestinationPath /
            f"{filename.replace('.csv', '')}_{segment_num}.csv",
            index=False)


if __name__ == '__main__':
    for i in range(17):
        main(f"train_wave_{i}.csv")
