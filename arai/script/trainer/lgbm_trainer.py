...
class LGBMTrainer(AbstractTrainer):
    def __init__(self, logger, n_splits=5, seed=42, kwargs={}):
        super(LGBMTrainer, self).__init__(logger, n_splits, seed)
    ...
    def _fit(self, X_train, y_train, X_val, y_val, n_epochs):
        model = lgb.LGBMRegressor(n_estimators=n_epochs, **self.kwargs)
        model.fit(
            X_train,
            y_train,
