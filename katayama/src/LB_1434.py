...
def main():
    ...
    result_dict_lgb_mini = train_model_regression(X=X[columns],
                                             X_test=X_test[columns],
                                             y=y,
                                             params=params,
                                             folds=folds,
    ...
    # [661]	training's l1: 1.1473	valid_1's l1: 1.9004
    # [472]	training's l1: 1.322	valid_1's l1: 1.9485
    # [596]	training's l1: 1.21464	valid_1's l1: 1.8541
    # [770]	training's l1: 1.05954	valid_1's l1: 1.91608
    # [822]	training's l1: 1.02246	valid_1's l1: 1.91461
    # CV mean score: 1.9067, std: 0.0307.

    print(np.mean(result_dict_lgb_mini['scores']))
    plot_result(result_dict_lgb_mini, y)
    print(len(y))

    # idea1
    # n_top = 1000, early_stopping_rounds = 10
    # n_top = 1000, early_stopping_rounds = 15
    ...
    # LB = 1.434
    # [420]	training's l1: 1.38161	valid_1's l1: 1.91503
    # [352]	training's l1: 1.45831	valid_1's l1: 1.95975
    # [454]	training's l1: 1.35697	valid_1's l1: 1.86339
    # [518]	training's l1: 1.27688	valid_1's l1: 1.92574
    # [366]	training's l1: 1.44128	valid_1's l1: 1.93756
    # CV mean score: 1.9203, std: 0.0321.


    submission_mini = create_submission_file(result_dict_lgb_mini['prediction'])
    #submission_mini.to_csv(DATA_DIR / f'output/submission_{slide_size}_top{n_top}_esr5.csv')
    plot_result(result_dict_lgb_mini, y)
    ...
    print("\n", n_top)
    print(np.mean(np.abs(LB_1456["time_to_failure"].values - submission_mini["time_to_failure"].values)))
    print(np.mean(np.abs(LB_1450["time_to_failure"].values - submission_mini["time_to_failure"].values)))
    print(np.mean(np.abs(LB_1438["time_to_failure"].values - submission_mini["time_to_failure"].values)))
    print(np.mean(np.abs(LB_1436["time_to_failure"].values - submission_mini["time_to_failure"].values)))
    print(np.mean(np.abs(LB_1434["time_to_failure"].values - submission_mini["time_to_failure"].values)))
    print()
