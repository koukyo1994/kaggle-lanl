...
class FeatureGenerator(object):
        ...
        if self.dtype == 'train':
            ...
            if self.chunk_size == 150000:
                for counter, df in enumerate(iter_df):
                    ...
                    del df
                    yield seg_id, x, y
            elif 10000 <= self.chunk_size and self.chunk_size < 150000:
                assert 150000/self.chunk_size, 'chunk size should be able to divide 150000.'

                df = pd.DataFrame()
                for _ in range(int(150000/self.chunk_size)):
                ...
                for sub_df in iter_df:
                    ...
                    # Update variables
                    df = df.iloc[self.chunk_size:, :].append(sub_df)
                    seg_id_num += 1
    ...
    def generate_2(self, denoising=False, random_slide=False):
        ...
        if self.dtype == 'train':
            ...
            if random_slide:
                np.random.choice(range(train.shape[0]), 24000, replace=False)
            else:
                start = 0
                end = 150000
                index_tuple_list = []
